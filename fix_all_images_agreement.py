...
# 2. Extract Getlands Logo and PM Signature from CertificateTemplate
with open('src/components/pdf/CertificateTemplate.tsx', 'r') as f:
    cert = f.read()

getlands_logo_match = re.search(r'src=\{`(data:image[^`]+)`\} style=\{styles\.logo\}', cert)
getlands_logo = getlands_logo_match.group(1) if getlands_logo_match else ''
if not getlands_logo:
    print("Could not find getlands logo in cert")

getlands_sig_match = re.search(r'src=\{`(data:image[^`]+)`\} style=\{styles\.signatureImage\}', cert)
getlands_sig = getlands_sig_match.group(1) if getlands_sig_match else ''
if not getlands_sig:
    print("Could not find getlands sig in cert")
