...
new_bank_form = r"""{activeTab === 'bank' && (
...
content = re.sub(bank_form_pattern, lambda m: new_bank_form, content, flags=re.DOTALL)

# Replace KYC Form
kyc_form_pattern = r"\{activeTab === 'kyc' && \(.*?\)\}"
new_kyc_form = r"""{activeTab === 'kyc' && (
...
content = re.sub(kyc_form_pattern, lambda m: new_kyc_form, content, flags=re.DOTALL)
